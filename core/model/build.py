...
def build_model(cfg):
    ...
    if cfg.CORRUPTION.DATASET in [
        ...
        if cfg.ADAPTER.NAME == "petal":
            base_model, base_mean_model, base_cov_model = load_model_bayes(
                cfg.MODEL.ARCH, cfg.CKPT_DIR, dataset, ThreatModel.corruptions
            )
            base_model, base_mean_model, base_cov_model = (
                base_model.cuda(),
                base_mean_model.cuda(),
                base_cov_model.cuda(),
            )
        ...
        base_model.fc = nn.Linear(
            in_features=base_model.fc.in_features, out_features=10
        )

        if os.path.exists(
            os.path.join(cfg.CKPT_DIR, cfg.CORRUPTION.DATASET, "source", "resnet18.pth")
        ):
            base_model.load_state_dict(
                torch.load(
                    os.path.join(
                        cfg.CKPT_DIR, cfg.CORRUPTION.DATASET, "source", "resnet18.pth"
                    )
                )
            )
        base_model.cuda()
