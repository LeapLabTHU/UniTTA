...
class RobustBN1d(MomentumBN):
    def forward(self, x):
        if self.training:
            ...
            self.source_mean, self.source_var = (
                deepcopy(mean.detach()),
                deepcopy(var.detach()),
            )
...
class BalancedBNV5(nn.Module):
    def __init__(
        self, bn_layer: nn.BatchNorm2d, num_classes=1, momentum_a=1e-01, gamma=0.0
    ):
        super().__init__()
        self.num_features = bn_layer.num_features
        ...
        self.bias = deepcopy(bn_layer.bias)
        local_mean = (
            deepcopy(bn_layer.running_mean)[None, ...].expand(num_classes, -1).clone()
        )
        local_var = (
            deepcopy(bn_layer.running_var)[None, ...].expand(num_classes, -1).clone()
        )
...
class BalancedRobustBN1dV5(BalancedBNV5):
    def forward(self, x):
        ...
        if label is not None:
            balanced_bn.update_statistics_1d_v5(
                self.local_mean,
                self.local_var,
                self.global_mean,
                self.global_var,
            ...
            if self.training:
                b_var, b_mean = torch.var_mean(
                    x, dim=[0, 2], unbiased=False, keepdim=False
                )  # (C,)
                self.global_mean = (
                    1 - self.momentum
                ) * self.global_mean + self.momentum * b_mean
                self.global_var = (
                    1 - self.momentum
                ) * self.global_var + self.momentum * b_var
        x = (x - self.global_mean[None, :, None]) / torch.sqrt(
            self.global_var[None, :, None] + self.eps
        )
...
class BalancedRobustBN2dV5(BalancedBNV5):
    def forward(self, x):
        ...
        label = self.label
        if label is not None:
            balanced_bn.update_statistics_2d_v5(
                self.local_mean,
                self.local_var,
                self.global_mean,
                self.global_var,
        ...
        x = (x - self.global_mean[None, :, None, None]) / torch.sqrt(
...
class BalancedRobustBN2dEMA(BalancedBNEMA):
    def forward(self, x):
        ...
        label = self.label
        if label is not None:
            balanced_bn.update_statistics_2d_ema(
                self.local_mean,
                self.local_var,
                self.global_mean,
                self.global_var,
