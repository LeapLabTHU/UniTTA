...
class AvgResultProcessor:
    ...
    def calculate(self):
        for c in range(self.num_class):
            ...
            self.result_per_class[key] = (
                self.acc_per_class[key] / self.count_per_class[key]
                if self.count_per_class[key] > 0
                else 0.0
            )
    ...
    def info(self):
        ...
        for c in range(self.num_class):
            ...
            t.add_row(
                [
                    self.label2name[key],
                    f"{self.result_per_class[key] * 100:.2f}",
                    f"{(1 - self.result_per_class[key]) * 100:.2f}",
                ]
            )
        ...
        per_domain_err = [
            f"{(1 - self.result_per_class[c]) * 100:.2f}" for c in range(self.num_class)
        ]
