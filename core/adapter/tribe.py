...
class TRIBE(BaseAdapter):
    def __init__(self, cfg, model, optimizer):
        ...
        for (name1, param1), (name2, param2) in zip(
            self.model.named_parameters(), self.aux_model.named_parameters()
        ):
    ...
    def forward_and_adapt(self, batch_data, model, optimizer):
        ...
        # return self.update_model(model, optimizer, batch_data, ema_out)
    ...
    def update_model(self, model, optimizer, batch_data, logit):
        ...
        entropy_mask = entropy < self.cfg.ADAPTER.TRIBE.H0 * math.log(
            self.cfg.CORRUPTION.NUM_CLASS
        )

        l_sup = torch.nn.functional.cross_entropy(
            stu_sup_out, ema_sup_out.argmax(dim=-1), reduction="none"
        )[entropy_mask].mean()
        # l_sup = torch.nn.functional.cross_entropy(
        #    stu_sup_out, ema_sup_out, reduction="none"
        # )[entropy_mask].mean()
        ...
        l_reg = (
            self.cfg.ADAPTER.TRIBE.LAMBDA
            * torch.nn.functional.mse_loss(
                ema_sup_out, source_anchor, reduction="none"
            )[entropy_mask].mean()
        )
