...
def load_imagenetc(
    ...
    data_folder_path = (
        Path(data_dir)
        / CORRUPTIONS_DIR_NAMES[BenchmarkDataset.imagenet]
        / corruptions[0]
        / str(severity)
    )
    print(f"Loading from {data_folder_path}")
...
def load_imagenet3dcc(
    ...
    data_folder_path = (
        Path(data_dir)
        / CORRUPTIONS_DIR_NAMES[BenchmarkDataset.imagenet_3d]
        / corruptions[0]
        / str(severity)
    )
...
def load_corruptions_cifar(
    ...
    for corruption in corruptions:
        ...
        print(f"Loading {corruption_file_path}...")
        # images_all = np.load(corruption_file_path)
        images_all = load(corruption_file_path)
